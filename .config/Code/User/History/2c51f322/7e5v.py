...
class Animation(Scene):
    ...
    def divided_land_3(self):
        ...
        for i in range(grid_size):
            for j in range(grid_size):
                ...
                txt = Text(str(count)).move_to(sq.get_center())
    ...
    def construct(self):
        ...
        Dsa_explanation = Paragraph(
            "Now we can see that each piece of land is affected by its neighbours, "
            "so we could use this information to check the health of the soil.",
            alignment="center",
            line_spacing=0.8
        ).scale(0.6) 
